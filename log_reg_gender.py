...
def get_data() -> pd.DataFrame:
    return pd.read_sql(
        ...
        "SELECT TOP (500000) ATTENDANCE_STATUS, SEX\
        FROM [MRT_KMCCG].[dbo].[FSUS_OPA_MAIN] AS OP\
        WHERE REP_DATE >= '20190101' AND SEX IS NOT NULL AND ATTENDANCE_STATUS IS NOT NULL AND ATTENDANCE_STATUS IN (3,7,5,6)",
        cnxn)
...
co_eff_df['feature'] = co_eff_df['feature'].str.replace("GENDER_DESC_","")
